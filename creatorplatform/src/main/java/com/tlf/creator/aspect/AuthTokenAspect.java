            ...
            if (verify) {
                ...
                if (!StringUtils.isBlank(role_name)) {
                    if (role != null && role.equals(role_name)) {
                        ...
                        return new JsonResult(Constants.CODE_BAD, "权限不足", null, null);
                    }
                } else {
                    return pjp.proceed();
                }
            } else {
                return new JsonResult(Constants.CODE_BAD, "token校验失败", null, null);
            }
        } else {
            return new JsonResult(Constants.CODE_BAD, "token为空", null, null);
        }
    }
