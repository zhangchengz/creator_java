...
public class FileAPI1 {
    ...
    protected static final Logger logger = LoggerFactory.getLogger(FileAPI1.class);
    ...
    public ResultResp uploadFile(HttpServletRequest request) throws IOException {
        ...
        ResponseEntity<JsonResult> s = restTemplate.postForEntity(Constants.CREATOR_COURSE1 + "upload_file", requestEntity, JsonResult.class);
    ...
    public ResultResp getResource(@RequestBody ResourceReq req, HttpServletRequest request) {
        ...
        req.setRole(role);
        HttpEntity<Object> requestEntity = new HttpEntity<>(req, requestHeaders);
        ResponseEntity<JsonResult> s = restTemplate.postForEntity(Constants.CREATOR_COURSE1 + "get_resource_by_id", requestEntity, JsonResult.class);
        logger.info("remote call " + s);
        JsonResult resp = s.getBody();
        if (resp.getCode() == 0) {
            return new ResultResp(Constants.CODE_SUCCESS, "SUCCESS", null, resp.getObject());
    ...
    public ResultResp testUpload(HttpServletRequest request) throws IOException {
        ...
        req.setRole(role);
        HttpEntity<Object> requestEntity = new HttpEntity<>(req, requestHeaders);
        ResponseEntity<JsonResult> s = restTemplate.postForEntity(Constants.CREATOR_COURSE1 + "test_upload_file", requestEntity, JsonResult.class);
        logger.info("remote call " + s);
        JsonResult resp = s.getBody();
        if (resp.getCode() == 0) {
            return new ResultResp(Constants.CODE_SUCCESS, "SUCCESS", null, resp.getObject());
    ...
    public Object download(HttpServletRequest request, @RequestBody DownloadReq req) {
        ...
        req.setRole(role);
        HttpEntity<Object> requestEntity = new HttpEntity<>(req, requestHeaders);
        ResponseEntity<byte[]> resp = restTemplate.postForEntity(Constants.CREATOR_COURSE1 + "download", requestEntity, byte[].class);
        return resp;
    }
}
