package com.tlf.creatorcourse.exception;
...
public class ExceptionController {
    ...
    public JsonResult handle401(TokenErrorException e) {
        return new JsonResult(Constants.CODE_BAD, e.getMsg(), null);
    }
    ...
    public JsonResult handle401() {
        return new JsonResult(Constants.CODE_BAD, "token校验异常", null);
    }

    @ResponseStatus(HttpStatus.UNAUTHORIZED)
    @ExceptionHandler(TokenExpiredException.class)
    public JsonResult tokenExpired() {
        return new JsonResult(Constants.CODE_BAD, "token已过期", null);
    }
    // 捕捉其他所有异常
